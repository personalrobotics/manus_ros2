...
class ManusSubscriber(Node):
    ...
    def listener_callback(self, msg):
        ...
        for ergo_data in msg.ergonomics:
            if ergo_data.type == "ThumbMCPSpread":
                # MCP range: [-5, 35]
                self.fingers[0] = max(0, min((ergo_data.value + 5) / 40 * 100, 100))
            elif ergo_data.type == "ThumbMCPStretch":
                # MCP range: [-10, 60]
                self.fingers[1] = (ergo_data.value - 40) * -1
                self.fingers[1] = max(0, min((self.fingers[1] + 10) / 70 * 100, 100))
            elif ergo_data.type == "IndexMCPStretch":
                # MCP range: [5, 90]
                self.fingers[2] = max(0, min((ergo_data.value - 5) / 85 * 100, 100))
            elif ergo_data.type == "MiddleMCPStretch":
                # MCP range: [10, 85]
                self.fingers[3] = max(0, min((ergo_data.value - 10) / 75 * 100, 100))
                ...
                # MCP range: [5, 70]
                self.fingers[5] = max(0, min((ergo_data.value - 5) / 65 * 100, 100))
